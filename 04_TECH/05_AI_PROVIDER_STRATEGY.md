...
- Sidebar composer picker now shows a local-only `Routing` hint so users can see whether the typed request is expected to use tab metadata, current-page Page Agent, selected text, selected page region, selected tabs/group context, or the internal Search Tool. This first slice is explanatory UI only: it does not switch provider, call AI/search, request permissions, read page content, upload data, or add a model marketplace.

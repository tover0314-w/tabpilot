...
Placeholders to replace: `[Developer name]`, `[support email]`, `[website URL]`
...
TabMosaic AI is a Chrome extension that organizes open work tabs into Chrome native tab groups, helps review safe duplicate tabs, and provides sidebar/dashboard controls for Undo, Restore, refinement, and user-triggered current-tab summary.
...
We use this data only to provide and improve TabMosaic AI's user-facing features: tab grouping, duplicate review, Undo/Restore, local rules, current-tab summary, local diagnostics, and safety validation.

## Data Storage

TabMosaic AI stores settings, rules, Undo/Restore snapshots, optional API settings, redacted local error summaries, and duplicate close safety counts in chrome.storage.local on the user's device. The extension does not provide cloud sync or account storage in the current build.

Users can clear local TabMosaic data from Dashboard -> Settings -> Clear Local Data. This removes local rules, API key/settings, recent results, Undo/Restore snapshots, chat drafts, first-run privacy acceptance, local error summaries, and local duplicate safety audit counts. It does not close tabs, move tabs, delete browser history, delete cookies, or touch cloud account data.

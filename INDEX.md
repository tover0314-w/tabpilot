...
- `tools/prepare_launch_unblock_packet.js`：生成 ignored 的 launch unblock 包，把最终上线门禁、GitHub billing/CI 动作、D-L03 到 D-L14 决策模板和真实 profile QA 下一步合成一个 README / HTML / checklist / JSON；不批准、不发布、不读取私密浏览数据；支持 `--self-test` 和可选 `--include-remote-ci`。

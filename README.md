...
它会加载 extension，打开本地 Manual QA Checklist、合成 QA tabs、sidepanel 和 dashboard 页面，只使用 `artifacts/manual-qa-profiles/` 下的一次性 Chrome profile，不读取你的真实 Chrome profile、真实 tabs 或 `.env.local`。`--self-test` 会打开后自动关闭并清理。

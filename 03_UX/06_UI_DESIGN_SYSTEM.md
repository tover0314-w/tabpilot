...
- ✅ AI 不可绕过确认直接做破坏性动作
- ✅ 消息卡片允许极克制的玻璃微渐变,用于让聊天层级更自然
- ✅ Sidebar composer 只有一个输入面: 当前 tab / selected tabs / group 上下文作为输入框内的 inline chip,不能再漂在输入框上方变成状态条
- ✅ Agent tool state 只能像 AI 对话里的轻量状态提示,不能像 dashboard 面板、调试卡片或设置区块
- ✅ Sidebar 长对话只能有一个主滚动: 消息流滚动到最新消息,composer 固定在底部,不允许消息卡片或输入框被横向裁切
- ❌ 不用霓虹、强色渐变、营销式紫蓝渐变、毛玻璃滥用
...
CONFIRMED BY DISCUSSION / IMPLEMENTATION:

Sidebar Agent 消息卡片允许使用克制的玻璃微渐变:

```text
AI message: transparent white → soft neutral green-grey
User message: pale sage → translucent white
Action summary: transparent white → very soft neutral tint
```
...
- 不使用紫蓝营销渐变
- 不使用高饱和色
- 不使用独立光球、bokeh、装饰性 blob
- 渐变只服务消息层级和可读性,不能抢过内容
```
...
- 时间用 mono 9px 灰字
- 当前页面问答参考 Notion AI sidebar: 上下文靠近 composer,回答是普通 AI 气泡,不要变成独立 summary/report 面板
- 输入框使用轻量 textarea composer: Enter 发送,Shift+Enter 换行,发送按钮用图标而不是大文字按钮
